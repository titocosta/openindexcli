...
const TOKEN_REGISTRY = tokenRegistryData;
// Function to resolve token symbol to address
function resolveTokenAddress(tokenInput, chain) {
    ...
    if (!address) {
        throw new Error(`Token "${tokenInput}" not found in ${chain.toUpperCase()} registry. ` +
...
async function resolveUsernameToAddress(recipient) {
    // If it's already an address (starts with 0x), return it
...
program
    ...
    const selectedChain = opts.chain;
    if (selectedChain && selectedChain !== "eth") {
        ...
        Object.entries(TOKEN_REGISTRY).forEach(([chain, tokens]) => {
            console.log(`${CHAIN_CONFIGS[chain].name} (${chain.toUpperCase()}):`);
...
program
    ...
    try {
        ...
        const encrypted = await EthCrypto.encryptWithPublicKey(recipientPubKey, message);
...
const hashUsername = (name) => createHash('sha256').update(name.toLowerCase()).digest('hex');
